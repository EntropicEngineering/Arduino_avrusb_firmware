 ...
 *  the device.
 */
const MS_OS_20_Descriptor_t PROGMEM
MS_OS_20_Descriptor =
    {
        .Header =
            {
...
const MS_OS_20_Descriptor_WebUSB_t PROGMEM
MS_OS_20_Descriptor_WebUSB =
    {
        .Header =
            {
                .Length = CPU_TO_LE16(10),
 ...
 */
const WebUSB_URL_Descriptor_t PROGMEM
WebUSB_LandingPage = WEBUSB_URL_DESCRIPTOR(1, u8"www.entropicengineering.com");
                        ...
                        case WebUSB_RTYPE_GetURL:
                            switch (USB_ControlRequest.wValue) {
                                case WEBUSB_LANDING_PAGE_INDEX:
                                    ...
                                    Endpoint_Write_Control_PStream_LE(&WebUSB_LandingPage,
                                                                      WebUSB_LandingPage.Header.Size);
                ...
                case MS_OS_20_VENDOR_CODE:
                    switch (USB_ControlRequest.wIndex) {
                        case MS_OS_20_DESCRIPTOR_INDEX:
                            ...
                            /* Write the descriptor data to the control endpoint */
                            if (WebUSB_Enabled) {
                                Endpoint_Write_Control_PStream_LE(&MS_OS_20_Descriptor_WebUSB,
                                                                  MS_OS_20_Descriptor_WebUSB.Header.TotalLength);
                            } else {
                                Endpoint_Write_Control_PStream_LE(&MS_OS_20_Descriptor,
                                                                  MS_OS_20_Descriptor.Header.TotalLength);
                            }
                            /* Release the endpoint after transaction. */
                            Endpoint_ClearStatusStage();
                            break;
...
void EVENT_CDC_Device_LineEncodingChanged(USB_ClassInfo_CDC_Device_t *const CDCInterfaceInfo) {
    ...
    UBRR1 = (CDCInterfaceInfo->State.LineEncoding.BaudRateBPS == 57600 ||
             CDCInterfaceInfo->State.LineEncoding.BaudRateBPS == 300)
    ...
    UCSR1A = (CDCInterfaceInfo->State.LineEncoding.BaudRateBPS == 57600 ||
              CDCInterfaceInfo->State.LineEncoding.BaudRateBPS == 300) ? 0 : (1 << U2X1);
